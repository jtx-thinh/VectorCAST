...
/* The function call data is the same as the statement data, except it has an
 * extra space, and then an extra 'C' character at the end.
 */
...
/* The amount of space needed to write a line of function data is,
 * The size of the unit id, plus a space, plus the size of the
 * unique id, plus the NUL character.
 */
#define VCAST_FUNCTION_DATA_BUF_SIZE 12
...
#ifdef __CUDA_ARCH__
   {
     char *ptr = &covdata[(bit_offset + cond_val) >> 5];
     int  val  = 1 << ((bit_offset + cond_val) % 32);
     covdata[(bit_offset + cond_val) >> 5] = atomicOr ( (int*)ptr, val );
   }
   {
     char *ptr = &covdata[(cond_result_start + cond_val) >> 5];
     int  val  = actual_result << ((cond_result_start + cond_val) % 32);
     covdata[(cond_result_start + cond_val) >> 5] = atomicOr ( (int*)ptr, val );
   }
#else
   covdata[(bit_offset + cond_val) >> 3] |= (1 << ((bit_offset + cond_val) % 8));
   covdata[(cond_result_start + cond_val) >> 3] |= (actual_result << ((cond_result_start + cond_val) % 8));
...
void VCAST_DUMP_COVERAGE_DATA (void) {
  ...
  if ( error_id == cudaSuccess ) {
     ...
     for (dev=0; dev<deviceCount; ++dev ) {
        ...
        char prompt[13];
        unsigned family = VCAST_CUDA_FAMILY ( properties.major, properties.minor );
